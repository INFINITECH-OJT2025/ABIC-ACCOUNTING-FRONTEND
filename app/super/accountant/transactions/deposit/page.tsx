...
export default function DepositPage() {
  ...
  const handleCreateDeposit = async () => {
    ...
    if (!formData.amount || !formData.amount.toString().trim()) {
      setCreateDepositFailMessage("Amount is required");
      setShowCreateDepositFail(true);
      return;
    }
    ...
    if (isNaN(parsedAmount) || parsedAmount < 0.01) {
      setCreateDepositFailMessage("Amount must be at least ₱0.01");
      setShowCreateDepositFail(true);
      return;
    }
    
    if (parsedAmount > 999999999.99) {
      setCreateDepositFailMessage("Amount cannot exceed ₱999,999,999.99");
      setShowCreateDepositFail(true);
      return;
    }
