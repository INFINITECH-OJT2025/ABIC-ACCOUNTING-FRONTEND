...
export default function BanksAccountsPage() {
  ...
  function onCreate(e: React.FormEvent) {
    e.preventDefault();
    if (!form.name.trim() || !form.bank.trim() || !form.accountName.trim() || !form.accountNumber.trim()) return;
    ...
    setForm({ name: "", bank: "", accountName: "", accountNumber: "" });

    closeCreatePanel();

    setSuccessTitle("Account Created Successfully");
    setSuccessMessage("Bank account has been created.");
    setShowSuccess(true);
  }
  ...
  function openEditModal(item: BankAccount) {
    setEditing(item);
    setEditBankQuery("");
    setEditForm({ name: item.name, bank: item.bank, accountName: item.accountName, accountNumber: item.accountNumber });
  }
  ...
  function saveEdit() {
    if (!editing) return;
    if (!editForm.name.trim() || !editForm.bank.trim() || !editForm.accountName.trim() || !editForm.accountNumber.trim()) return;
    ...
    setSuccessMessage("Bank account changes have been saved.");
    setShowSuccess(true);
  }

  const tableCols =
    "minmax(240px, 1.8fr) minmax(180px, 1.2fr) minmax(180px, 1.2fr) minmax(160px, 1.1fr) 110px 120px";

  return (
    <div className="bg-white">
      <main className="mx-auto max-w-7xl px-4 py-8">
        <div className="mt-7">
          <section className="rounded-2xl bg-white shadow-sm border" style={{ borderColor: BORDER }}>
            <div className="p-5">
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0">
                  <h2 className="text-base font-extrabold text-neutral-900">Bank Accounts List</h2>
                  <p className="mt-1 text-xs text-neutral-600">Manage bank master data and records.</p>
                </div>

                <button
                  onClick={() => setShowCreate(true)}
                  className="inline-flex items-center justify-center gap-2 rounded-xl px-4 text-xs font-semibold text-white hover:opacity-95 flex-shrink-0"
              ...
              <div className="mt-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className={
                        ...
                        (statusFilter === "all" ? "bg-[#7a0f1f] text-white border-transparent" : "bg-white text-neutral-700 hover:bg-neutral-50")
                      }
                      style={{ borderColor: statusFilter === "all" ? undefined : BORDER }}
                    >
                      All
                    </button>
                    <button
                      ...
                      className={
                        "px-3 py-1.5 text-xs font-semibold rounded-lg border transition-colors " +
                        (statusFilter === "Active" ? "bg-[#7a0f1f] text-white border-transparent" : "bg-white text-neutral-700 hover:bg-neutral-50")
                      }
                      style={{ borderColor: statusFilter === "Active" ? undefined : BORDER }}
                    >
                      Active
                    </button>
                    <button
                      ...
                      className={
                        "px-3 py-1.5 text-xs font-semibold rounded-lg border transition-colors " +
                        (statusFilter === "Inactive" ? "bg-[#7a0f1f] text-white border-transparent" : "bg-white text-neutral-700 hover:bg-neutral-50")
                      }
                      style={{ borderColor: statusFilter === "Inactive" ? undefined : BORDER }}
                    >
                      Inactive
                    </button>
                  </div>
                ...
                <div className="flex items-center gap-2 justify-end">
                  <button
                    type="button"
                    onClick={onRefresh}
                    className="inline-flex items-center justify-center rounded-xl border bg-white hover:bg-neutral-50"
                    style={{ borderColor: BORDER, height: 40, width: 40, color: "#111" }}
                    title="Refresh"
                    aria-label="Refresh"
                  >
                    <Icons.Refresh />
                  ...
                  <div className="flex items-center rounded-xl overflow-hidden" style={{ border: `1px solid ${BORDER}` }}>
                    <button
                      ...
                      className={"px-3 " + (viewMode === "cards" ? "bg-[#7a0f1f] text-white" : "bg-white text-neutral-700 hover:bg-neutral-50")}
                    ...
                    <button
                      ...
                      className={"px-3 " + (viewMode === "table" ? "bg-[#7a0f1f] text-white" : "bg-white text-neutral-700 hover:bg-neutral-50")}
                      style={{ height: 40, width: 44 }}
                      title="Table View"
              ...
              {filtered.length === 0 ? (
                  ...
                  {filtered.map((a) => (
                    <div key={a.id} className="rounded-2xl bg-white border shadow-sm p-4" style={{ borderColor: BORDER }}>
                      <div className="flex items-start justify-between mb-3">
                        ...
                        <span
                          className={
                            "inline-flex items-center justify-center rounded-full px-3 text-[11px] font-extrabold " +
                            (a.status === "Active" ? "bg-emerald-100 text-emerald-800" : "bg-gray-200 text-gray-700")
                          }
                          style={{ height: 26 }}
                        >
                          {a.status}
                ...
                <div className="overflow-x-auto">
                  <div className="overflow-hidden rounded-2xl border min-w-[980px]" style={{ borderColor: BORDER }}>
                    <div className="grid bg-neutral-50 px-4 py-3 text-xs font-extrabold text-neutral-900" style={{ gridTemplateColumns: tableCols }}>
                    ...
                    {filtered.map((a) => (
                      <div
                        key={a.id}
                        className="grid items-center px-4 py-3 text-sm border-t hover:bg-neutral-50/70"
                        style={{ borderColor: BORDER, color: "#111", gridTemplateColumns: tableCols }}
                      >
                        <div className="min-w-0 flex items-center gap-3">
                          <div className="w-10 h-10 rounded-lg bg-[#7a0f1f]/10 grid place-items-center flex-shrink-0">
                            <span className="text-sm font-extrabold text-[#7a0f1f]">{a.name.charAt(0).toUpperCase()}</span>
                          </div>
                          <div className="min-w-0">
                            <div className="font-extrabold text-neutral-900 truncate leading-5">{a.name}</div>
                            <div className="text-[11px] text-neutral-600 truncate leading-4">{a.accountName}</div>
                          </div>
                        </div>

                        <div className="min-w-0 text-neutral-900 truncate">
                          <div className="font-semibold truncate">{a.bank}</div>
                          <div className="text-[11px] text-neutral-600 truncate">Bank</div>
                        </div>

                        <div className="min-w-0 text-neutral-900 truncate">
                          <div className="font-semibold truncate">{a.accountName}</div>
                          <div className="text-[11px] text-neutral-600 truncate">Account name</div>
                        </div>

                        <div className="min-w-0 text-neutral-900 truncate">
                          <div className="font-semibold truncate">{a.accountNumber}</div>
                          <div className="text-[11px] text-neutral-600 truncate">Account number</div>
                        </div>

                        <div className="min-w-0 text-neutral-900">
                          <div className="font-semibold">{a.createdAt}</div>
                          <div className="text-[11px] text-neutral-600">Created</div>
                        </div>
                        ...
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <button
                            type="button"
                            onClick={() => openEditModal(a)}
                            className="inline-flex items-center gap-2 rounded-xl px-3 text-xs font-extrabold text-white hover:opacity-95"
          ...
          >
            <div className="flex-shrink-0 flex items-center justify-between p-4 bg-gradient-to-r from-[#800020] via-[#A0153E] to-[#C9184A] text-white">
              <div>
                ...
                <p className="text-sm text-white/90 mt-0.5">Fill in the details below to add a bank account.</p>
              </div>
              <button
                type="button"
                onClick={closeCreatePanel}
      ...
      {editing && (
        ...
        >
          ...
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                <h3 className="text-lg font-bold text-[#5f0c18]">Edit Bank Account</h3>
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="start" className="w-[var(--radix-dropdown-menu-trigger-width)]">
        <div className="px-2 py-2" onKeyDown={(e) => e.stopPropagation()}>
          <Input value={query} onChange={(e) => setQuery(e.target.value)} placeholder={inputPlaceholder} className="h-9" autoFocus />
        </div>
        <DropdownMenuSeparator />
        ...
        {!canAdd && filtered.length === 0 && <div className="px-2 py-2 text-sm text-neutral-500">No banks</div>}
      </DropdownMenuContent>
    </DropdownMenu>
  );
}