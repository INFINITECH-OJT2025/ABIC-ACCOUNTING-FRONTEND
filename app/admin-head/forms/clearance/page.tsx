...
import {
  Save, Edit3, ArrowLeft, Printer, Lock, ChevronLeft, ChevronRight, Check, Trash2, Plus, Target
} from 'lucide-react'
...
const INITIAL_CLEARANCE_TASKS = [
  { id: 1, task: 'Return of Company ID and HMO Card', status: 'DONE' },
  { id: 2, task: 'Return of Company Laptop and Accessories', status: 'DONE' },
  { id: 3, task: 'Turnover of Departmental Keys and Files', status: 'PENDING' },
  { id: 4, task: 'Settlement of Petty Cash and Advances', status: 'DONE' },
  { id: 5, task: 'Exit Interview Conducted', status: 'DONE' },
];
...
export default function ClearanceChecklistPage() {
  ...
  const [tasks, setTasks] = useState(INITIAL_CLEARANCE_TASKS)
  const [open, setOpen] = useState(false)
  const [currentIndex, setCurrentIndex] = useState(0)
  const [employeeInfo, setEmployeeInfo] = useState(CLEARANCE_DB[currentIndex])
  ...
  const handleNext = () => {
    if (currentIndex < CLEARANCE_DB.length - 1) {
      const nextIdx = currentIndex + 1;
      setCurrentIndex(nextIdx);
      setEmployeeInfo(CLEARANCE_DB[nextIdx]);
    }
  }
  ...
  const handlePrev = () => {
    if (currentIndex > 0) {
      ...
      setEmployeeInfo(CLEARANCE_DB[prevIdx]);
    }
  }
    ...
    <div className="min-h-screen bg-slate-50/50 pb-12 font-sans">
      <header className="bg-gradient-to-r from-[#4A081A] via-[#630C22] to-[#7B0F2B] text-white shadow-lg p-8 mb-8">
        <div className="max-w-[1600px] mx-auto flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          <div className="space-y-4">
            ...
            {/* UPDATED BLUE CIRCLE AREA: Includes Percentage Status */}
                ...
                <PopoverContent className="w-[300px] p-0 rounded-xl border-none shadow-2xl">
                  <Command>
                    ...
                    <CommandList>
                      ...
                      <CommandGroup>
                        {CLEARANCE_DB.map((emp, index) => (
                          <CommandItem key={emp.id} onSelect={() => { setCurrentIndex(index); setEmployeeInfo(CLEARANCE_DB[index]); setOpen(false); }}>
      ...
      <main className="max-w-[1600px] mx-auto p-8 relative">
        ...
        <div className="absolute left-0 top-[25%] -translate-x-1/2 z-10">
          <Button onClick={handlePrev} disabled={currentIndex === 0} size="icon" className="rounded-full h-12 w-12 bg-white shadow-xl text-[#a0153e] border-none hover:bg-rose-50 transition-colors">
            <ChevronLeft className="h-6 w-6" />
          </Button>
        </div>
        <div className="absolute right-0 top-[25%] translate-x-1/2 z-10">
          <Button onClick={handleNext} disabled={currentIndex === CLEARANCE_DB.length - 1} size="icon" className="rounded-full h-12 w-12 bg-white shadow-xl text-[#a0153e] border-none hover:bg-rose-50 transition-colors">
        ...
        <Card className="rounded-[2rem] border-none shadow-2xl overflow-hidden bg-white mb-8">
          <div className="grid grid-cols-1 md:grid-cols-4 divide-y md:divide-y-0 md:divide-x divide-slate-100">
            ...
            <div className="p-8">
              ...
              ) : (<p className="text-lg font-semibold text-slate-700">{employeeInfo.department || 'Studio'}</p>)}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-slate-100 border-t border-slate-100">
            <div className="p-8">
              <p className="text-[11px] font-bold text-[#a0153e] uppercase tracking-[0.2em] mb-3">Resignation Date</p>
              {editMode ? (
              ...
              ) : (<p className="text-lg font-semibold text-slate-700">{new Date(employeeInfo.resignationDate).toLocaleDateString('en-US', { month: 'numeric', day: 'numeric', year: 'numeric' })}</p>)}
            </div>
            <div className="p-8">
              <p className="text-[11px] font-bold text-[#a0153e] uppercase tracking-[0.2em] mb-3">Last Day</p>
              {editMode ? (
              ...
              ) : (<p className="text-lg font-semibold text-slate-700">{new Date(employeeInfo.lastDay).toLocaleDateString('en-US', { month: 'numeric', day: 'numeric', year: 'numeric' })}</p>)}
            </div>
          </div>
        </Card>
          ...
          <div className="bg-slate-50/50 px-10 py-8 flex justify-between items-center border-t border-slate-100">
            ...
            <div className="flex gap-4">
              ...
              <Button onClick={() => setEditMode(!editMode)} className={cn("rounded-full px-12 h-12 font-bold shadow-xl transition-all", editMode ? "bg-emerald-600 hover:bg-emerald-700 text-white" : "bg-[#a0153e] hover:bg-[#801030] text-white")}>
                {editMode ? <><Save className="mr-2 h-4 w-4" /> Save Masterfile</> : <><Edit3 className="mr-2 h-4 w-4" /> Update Mode</>}
              </Button>
            </div>
          </div>
        </Card>
