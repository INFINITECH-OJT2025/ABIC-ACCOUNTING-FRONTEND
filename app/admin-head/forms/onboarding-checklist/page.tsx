...
import {
  Save, Edit3, ArrowLeft, Printer, Lock, ChevronLeft, ChevronRight, Check, Trash2, Plus, LayoutDashboard, ClipboardList
} from 'lucide-react'
...
export default function OnboardingChecklistPage() {
      ...
      try {
        ...
        if (positionsResponse.ok) {
          ...
          const names = Array.isArray(positionsData?.data)
            ? positionsData.data.map((item: NamedOption) => item.name).filter(Boolean)
            : []
        ...
        if (departmentsResponse.ok) {
          ...
          const names = Array.isArray(departmentsData?.data)
            ? departmentsData.data.map((item: NamedOption) => item.name).filter(Boolean)
            : []
  ...
  const toggleTaskStatus = async (id: number, checked: boolean) => {
    ...
    const updatedTasks = tasks.map(t => t.id === id
      ? { ...t, status: checked ? 'DONE' : 'PENDING', date: checked ? new Date().toLocaleDateString('en-CA') : '' }
      : t)
        ...
        <Card className="rounded-[2rem] border-none shadow-2xl bg-white overflow-hidden">
          ...
          <div className="bg-slate-50/50 px-10 py-8 flex justify-between items-center border-t border-slate-100">
            ...
            <div className="flex gap-4">
              ...
              >
                {editMode ? <><Save className="mr-2 h-4 w-4" /> {saving ? 'Saving...' : 'Save Updates'}</> : <><Edit3 className="mr-2 h-4 w-4" /> Update Mode</>}
              </Button>
            </div>
          </div>
        </Card>
